"""Accounting rules management for v2"""
...
class AccountingRulesManager:
    ...
    async def _query_db_rules(self) -> None:
        """Query the accounting rules in the db and update event_settings with them"""
        # In v2, we would use the AccountingRuleRepository
        # For now, using the legacy DB interface
    ...
    async def reset(self) -> None:
        await self.aggregators.reset()
        await self._query_db_rules()
        self.event_callbacks = await self.aggregators.get_accounting_callbacks()
        self.eth_staking_taxable_after_withdrawal_enabled = CachedSettings().get_settings().eth_staking_taxable_after_withdrawal_enabled  # noqa: E501
