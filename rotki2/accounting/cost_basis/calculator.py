"""Cost basis calculation methods and calculator"""
import heapq
import logging
...
from rotki2.accounting.cost_basis.structures import (
...
class CostBasisCalculator(CustomizableDateMixin):
    ...
    async def spend_asset(
        ...
        """
        Spends `amount` of the given asset at the given time and rate (in profit currency).
        If `taxable_spend` is False we also do not count any profit/loss on the spend.

        If `taxable_spend` is True then the relevant acquisition events are looked up
        and the PnL is calculated.
        """
        spend_event = AssetSpendEvent(
            timestamp=timestamp,
            location=location,
            amount=amount,
            rate=rate,
        ...
        asset_events = self.get_events(asset)
        asset_events.spends.append(spend_event)

        if taxable_spend is False:
            # If the spend is not taxable, then we don't calculate cost basis
            await self.reduce_asset_amount(
                originating_event_id=originating_event_id,
                asset=asset,
                amount=amount,
                timestamp=timestamp,
        ...
        cost_basis_info = asset_events.acquisitions_manager.calculate_spend_cost_basis(
