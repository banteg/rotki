"""Async EVM Node Inquirer for rotki2"""
import asyncio
import json
import logging
import random
...
class EvmNodeInquirer(ABC):
    """Async class containing generic functionality for querying EVM nodes
    
    The child class must implement the following methods:
    - _get_archive_check_data
    - _get_pruned_check_tx_hash
    - get_blocknumber_by_time (optional - base implementation uses blockscout/etherscan)
    
    The child class may optionally implement the following:
    - logquery_block_range
    - _additional_receipt_processing
    """
    ...
    async def maybe_connect_to_nodes(self, when_tracked_accounts: bool) -> None:
        ...
        accounts = await self.database.get_blockchain_accounts()
        ...
        if (
            ...
            rpc_nodes = await self.database.get_rpc_nodes(blockchain=self.blockchain, only_active=True)
            await self.connect_to_multiple_nodes(rpc_nodes)

    def connected_to_any_web3(self) -> bool:
        return len(self.web3_mapping) != 0

    def get_own_node_web3(self) -> AsyncWeb3 | None:
    ...
    async def get_multi_balance(
        ...
        result = await self.contract_scan.call(
            node_inquirer=self,
    ...
    async def get_historical_balance(
        ...
        try:
            result = await web3.eth.get_balance(address, block_identifier=block_number)
        except (
                aiohttp.ClientError,
                BlockchainQueryError,
    ...
    def _init_web3(self, node: NodeName) -> tuple[AsyncWeb3, str]:
        """Initialize a new AsyncWeb3 object based on a given endpoint"""
        ...
        provider = AsyncHTTPProvider(
            ...
            request_kwargs={'timeout': aiohttp.ClientTimeout(total=self.rpc_timeout)},
        )
        ...
        if is_connected:
            ...
            try:
                if connectivity_check:
                    try:
                        net_version = await web3.net.version  # version can be in hex too
                        network_id = int(net_version, 0)  # base 0 triggers the prefix check
                    ...
                    try:
                        current_block = await web3.eth.block_number  # pylint: disable=no-member
                    ...
                    except (aiohttp.ClientError, RemoteError) as e:
            ...
            if node.endpoint.endswith('llamarpc.com'):  # temporary. Seems to sometimes switch
                ...
                is_pruned, is_archive = await self.determine_capabilities(web3)
    ...
    async def connect_to_multiple_nodes(self, nodes: Sequence[WeightedNode]) -> None:
        """Connect to multiple nodes concurrently"""
        tasks = []
    ...
    async def _query(self, method: Callable, call_order: Sequence[WeightedNode], **kwargs: Any) -> Any:
        ...
        for node_idx, weighted_node in enumerate(call_order):
            ...
            if web3node is None:
                ...
                if node_info.name != ETHEREUM_ETHERSCAN_NODE_NAME:
                    success, _ = await self.attempt_connect(node=node_info)
                    if success is False:
            ...
            try:
                ...
                if asyncio.iscoroutinefunction(method):
                    result = await method(web3, **kwargs)
                else:
            ...
            except asyncio.TimeoutError as e:  # Add node to failed_to_connect_nodes to prevent repeatedly timing out on the same node.  # noqa: E501
                log.warning(
    ...
    async def _get_latest_block_number(self, web3: AsyncWeb3 | None) -> int:
        if web3 is not None:
    ...
    async def get_latest_block_number(self, call_order: Sequence[WeightedNode] | None = None) -> int:
    ...
    async def _get_block_by_number(self, web3: AsyncWeb3 | None, num: int) -> dict[str, Any]:
        ...
        block_data: MutableAttributeDict = MutableAttributeDict(await web3.eth.get_block(num))  # type: ignore # pylint: disable=no-member
    ...
    async def _get_code(self, web3: AsyncWeb3 | None, account: ChecksumEvmAddress) -> str:
        ...
        code = await web3.eth.get_code(account)
        return code.to_0x_hex()
    ...
    def _call_contract_etherscan(
        ...
        """
        # Note: This uses sync Web3 for encoding/decoding only
        web3 = Web3()
    ...
    async def _get_transaction_receipt(
        ...
        try:
            tx_receipt = await web3.eth.get_transaction_receipt(tx_hash)  # type: ignore
    ...
    async def maybe_get_transaction_receipt(
            self,
            tx_hash: EVMTxHash,
            call_order: Sequence[WeightedNode] | None = None,
            must_exist: bool = False,
    ...
    async def get_transaction_receipt(
        ...
        """
        tx_receipt = await self.maybe_get_transaction_receipt(
            call_order=call_order if call_order is not None else self.default_call_order(),
            tx_hash=tx_hash,
            must_exist=True,
        )
    ...
    async def _get_transaction_by_hash(
        ...
        if web3 is None:
            ...
            tx_data = await web3.eth.get_transaction(tx_hash)  # type: ignore
        if tx_data is None:
            if must_exist:  # fail, so other nodes can be tried
    ...
    async def maybe_get_transaction_by_hash(
            self,
            tx_hash: EVMTxHash,
            call_order: Sequence[WeightedNode] | None = None,
            must_exist: bool = False,
    ...
    async def get_transaction_by_hash(
        ...
        """
        result = await self.maybe_get_transaction_by_hash(
            call_order=call_order if call_order is not None else self.default_call_order(),
            tx_hash=tx_hash,
            must_exist=True,
        )
    ...
    async def _get_logs(
        ...
        if web3 is not None:
            events = await self._query_web3_get_logs(
                web3=web3,
    ...
    async def _query_web3_get_logs(
            self,
            web3: AsyncWeb3,
        ...
        until_block = await web3.eth.block_number if to_block == 'latest' else to_block
        events: list[dict[str, Any]] = []
        start_block = from_block
        ...
        while start_block <= until_block:
            ...
            try:
                new_events_web3: list[dict[str, Any]] = [dict(x) for x in await web3.eth.get_logs(filter_args)]  # noqa: E501
    ...
    async def multicall(
        ...
        for call_chunk in calls_chunked:
            multicall_result = await self.contract_multicall.call(
                node_inquirer=self,
    ...
    async def multicall_2(
        ...
        return await self.contract_multicall.call(
            node_inquirer=self,
    ...
    async def multicall_specific(
        ...
        output = await self.multicall(calls, True, call_order)
        if decode_result is False:
            return output
        return [contract.decode(x, method_name, arguments[0]) for x in output]

    async def get_multiple_erc20_contract_info(self, addresses: list[ChecksumEvmAddress]) -> None:
        ...
        for addresses_chunk in get_chunks(addresses, 8):  # chunk number seems to be highest that can work with etherscan url limit  # noqa: E501
            ...
            try:
                ...
                output = await self.multicall_2(require_success=False, calls=calls)
            except RemoteError:
    ...
    async def _query_token_contract(
            self,
            abi: ABI,
    ...
    async def get_erc20_contract_info(self, address: ChecksumEvmAddress) -> dict[str, Any]:
        """
        ...
        output = await self._query_token_contract(abi=self.contracts.erc20_abi, properties=ERC20_PROPERTIES, address=address)  # noqa: E501
        info = self._process_and_create_erc20_info(
            output=output,
            address=address,
        )
    ...
    async def get_erc721_contract_info(self, address: ChecksumEvmAddress) -> dict[str, Any]:
        """
        ...
        output = await self._query_token_contract(abi=self.contracts.erc721_abi, properties=ERC721_PROPERTIES, address=address)  # noqa: E501
    ...
    async def determine_capabilities(self, web3: AsyncWeb3) -> tuple[bool, bool]:
        ...
        """
        is_archive = await self._have_archive(web3)
        is_pruned = await self._is_pruned(web3)

        return is_pruned, is_archive

    async def get_contract_deployed_block(self, address: ChecksumEvmAddress) -> int | None:
        ...
        transaction, _ = await self.get_transaction_by_hash(deployed_hash)
        return transaction.block_number

    async def maybe_timestamp_to_block_range(
            self,
        ...
        if period.range_type == 'timestamps':
            return TimestampOrBlockRange(
                range_type='blocks',
                from_value=await self.get_blocknumber_by_time(ts=Timestamp(period.from_value)),
                to_value=await self.get_blocknumber_by_time(ts=Timestamp(period.to_value)),
            )
    ...
    async def get_blocknumber_by_time(
            self,
            ts: Timestamp,
    ...
    def logquery_block_range(
            self,
            web3: AsyncWeb3,  # pylint: disable=unused-argument
    ...
    async def _have_archive(self, web3: AsyncWeb3) -> bool:
        ...
        balance = await self.get_historical_balance(
    ...
    async def _is_pruned(self, web3: AsyncWeb3) -> bool:
        """Returns a boolean representing if the node is pruned or not."""
        try:
            tx = await web3.eth.get_transaction(self._get_pruned_check_tx_hash())  # type: ignore
        except (
                aiohttp.ClientError,
                Web3Exception,
                KeyError,
    ...
    async def ensure_cache_data_is_updated(
            self,
