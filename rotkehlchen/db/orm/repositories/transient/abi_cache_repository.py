...
class ABICacheRepository(BaseRepository[EvmAbiCache]):
    ...
    def get_abi(self, address: ChecksumEvmAddress) -> Optional[EvmAbiCache]:
        """Get ABI for address"""
        return self.get(address=address)
    
    def get_abi_string(self, address: ChecksumEvmAddress) -> Optional[str]:
    ...
    def get_entries_by_age(
        ...
        older_than: Optional[Timestamp] = None,
    ) -> list[EvmAbiCache]:
