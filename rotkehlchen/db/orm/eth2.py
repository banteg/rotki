...
from sqlalchemy import (
    CHAR, INTEGER, TEXT, Column, ForeignKey, UniqueConstraint,
...
class Eth2Validator(Base):
    ...
    validator_index: Mapped[Optional[int]] = mapped_column(INTEGER, unique=True)
    ...
    withdrawal_address: Mapped[Optional[str]] = mapped_column(TEXT)
    validator_type: Mapped[int] = mapped_column(INTEGER, nullable=False)
    activation_timestamp: Mapped[Optional[int]] = mapped_column(TimestampType)
    withdrawable_timestamp: Mapped[Optional[int]] = mapped_column(TimestampType)
    exited_timestamp: Mapped[Optional[int]] = mapped_column(TimestampType)
